...
class BasicController extends Controller
{
	...
	public function updateInnerImage(Request $request, $langid)
    {
        $img = $request->file('file');
        $allowedExts = array('jpg', 'png', 'jpeg');
        ...
        if ($validator->fails()) {
            $validator->getMessageBag()->add('error', 'true');
            return response()->json(['errors' => $validator->errors(), 'id' => 'inner image']);
        }
        ...
        if ($request->hasFile('file')) {
            ...
            $bs = BasicSetting::where('language_id', $langid)->firstOrFail();
            @unlink('assets/front/img/' . $bs->inner_image);
            $filename = uniqid() .'.'. $img->getClientOriginalExtension();
            $img->move('assets/front/img/', $filename);

            $bs->inner_image = $filename;
            $bs->save();
        }

        return response()->json(['status' => "success", 'image' => 'inner image']);
    }
    public function updateVideoLink(Request $request, $langid)
    {
		$bs = BasicSetting::where('language_id', $langid)->firstOrFail();
		$bs->video_link = $request->video_link;
        $bs->save();
        Session::flash('success', 'Page video link updated successfully!');
        return back();
}
    public function deleteInnerImage($langid)
    {
