...
@endif
  <div class="page-header">
    <h4 class="page-title">{{ __('trans.Service Categories') }}</h4>
        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                    ...
                    <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete" data-href="{{route('admin.scategory.bulk.delete')}}"><i class="flaticon-interface-5"></i> {{ __('trans.Delete') }}</button>
                </div>
            </div>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12">
              @if (count($scategorys) == 0)
                <h3 class="text-center">{{ __('trans.NO SERVICE CATEGORY FOUND') }}</h3>
              @else
                  ...
                  <table class="table table-striped mt-3">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ __('trans.Icon Image') }}</th>
                        ...
                        <tr>
                          ...
                          <td>
                              ...
                              <select class="form-control {{$scategory->feature == 1 ? 'bg-success' : 'bg-danger'}}" name="feature" onchange="document.getElementById('featureForm{{$scategory->id}}').submit();">
                                  <option value="1" {{$scategory->feature == 1 ? 'selected' : ''}}>{{ __('trans.Yes') }}</option>
                                  <option value="0" {{$scategory->feature == 0 ? 'selected' : ''}}>{{ __('trans.No') }}</option>
                              </select>
                              </form>
                          </td>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">{{ __('trans.Add Service Category') }}</h5>
          ...
          <form class="mb-3 dm-uploader drag-and-drop-zone modal-form" enctype="multipart/form-data" action="{{route('admin.scategory.upload')}}" method="POST">
            <div class="form-row px-2">
              <div class="col-12 mb-2">
                <label for=""><strong>{{ __('trans.Icon mage') }} **</strong></label>
              </div>
