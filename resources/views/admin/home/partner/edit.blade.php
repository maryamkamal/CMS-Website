...
@endif
  <div class="page-header">
    <h4 class="page-title">{{ __('trans.Edit Partner') }}</h4>
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="#">{{ __('trans.Edit Partner') }}</a>
      </li>
    </ul>
  </div>
  <div class="row">
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <div class="card-title d-inline-block">{{ __('trans.Edit Partner') }}</div>
          <a class="btn btn-info btn-sm @if(session('language')== "ar") float-left  @else float-right @endif d-inline-block" href="{{route('admin.partner.index') . '?language=' . request()->input('language')}}">
            ...
            <div class="col-lg-6 offset-lg-3">
              ...
              <form id="ajaxForm" class="" action="{{route('admin.partner.update')}}" method="post">
                ...
                <div class="form-group">
                  ...
                  <input type="text" class="form-control ltr" name="url" value="{{$partner->url}}" placeholder="{{__('trans.Enter URL of social media account')}}">
                  <p id="errurl" class="text-danger mb-0 em"></p>
                </div>
                <div class="form-group">
                  <label for="">{{ __('trans.Serial Number') }} **</label>
                  <input type="number" class="form-control ltr" name="serial_number" value="{{$partner->serial_number}}" placeholder="{{__('trans.Enter Serial Number')}}">
                  <p id="errserial_number" class="mb-0 text-danger em"></p>
                  <p class="text-warning"><small>{{ __('trans.Partner Serial Number Struc') }}</small></p>
                </div>
              </form>
            </div>
          </div>
