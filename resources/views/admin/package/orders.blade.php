        ...
        <div class="card-header">
            <div class="row">
                ...
                <div class="col-lg-6 offset-lg-2">
                    <button class="btn btn-danger float-right btn-md mr-2 d-none bulk-delete ml-2" data-href="{{route('admin.order.bulk.delete')}}"><i class="flaticon-interface-5"></i> {{ __('trans.delete') }}</button>
                  ...
                  <table class="table table-striped mt-3">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ __('trans.Gateway') }}</th>
                        <th scope="col">{{ __('trans.Package') }}</th>
                          ...
                          <td>
                            <form id="statusForm{{$order->id}}" class="d-inline-block" action="{{route('admin.orders.status')}}" method="post">
                              ...
                              " name="status" onchange="document.getElementById('statusForm{{$order->id}}').submit();">
                                <option value="0" {{$order->status == 0 ? 'selected' : ''}}>{{ __('trans.Pending') }}</option>
                                <option value="1" {{$order->status == 1 ? 'selected' : ''}}>{{ __('trans.Processing') }}</option>
                                <option value="2" {{$order->status == 2 ? 'selected' : ''}}>{{ __('trans.Completed') }}</option>
                                <option value="3" {{$order->status == 3 ? 'selected' : ''}}>{{ __('trans.Rejected') }}</option>
                              </select>
                            </form>
                          </td>
                          <td>
                          ...
                          <td>
                            <div class="dropdown">
                                <button class="btn btn-secondary dropdown-toggle btn-sm" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                  {{ __('trans.Select Action') }}
                                </button>
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                  <a href="#" class="editbtn dropdown-item" data-target="#mailModal" data-toggle="modal" data-email={{$order->email}}> {{ __('trans.Send Mail') }}</a>
                                  <a href="#" class="dropdown-item" data-toggle="modal" data-target="#detailsModal{{$order->id}}" target="_blank"> {{ __('trans.Details') }}</a>
                                  <a class="dropdown-item" target="_blank" href="{{asset('assets/front/invoices/'.$order->invoice)}}"> {{ __('trans.Invoice') }}</a>
