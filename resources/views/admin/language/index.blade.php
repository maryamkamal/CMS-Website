...
@endif
  <div class="page-header">
    <h4 class="page-title">{{ __('trans.Languages') }}</h4>
    ...
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-header">
          <div class="card-title d-inline-block">{{ __('trans.Languages') }}</div>
          <a href="#" class="btn btn-primary float-right" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> {{ __('trans.Add Language') }}</a>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12">
              @if (count($languages) == 0)
                <h3 class="text-center">{{ __('trans.NO LANGUAGE FOUND') }}</h3>
              @else
                  ...
                  <table class="table table-striped mt-3">
                    <thead>
                      <tr>
                        ...
                        <th scope="col">{{ __('trans.Code') }}</th>
                        <th scope="col">{{ __('trans.Appearance in Website') }}</th>
                        ...
                        <tr>
                          ...
                          <td>
                            @if ($language->is_default == 1)
                              <strong class="badge badge-success">{{ __('trans.Default') }}</strong>
                            @else
                              <form class="d-inline-block" action="{{route('admin.language.default', $language->id)}}" method="post">
                                @csrf
                                <button class="btn btn-primary btn-sm" type="submit" name="button">{{ __('trans.Make Default') }}</button>
                              </form>
                            @endif
                          </td>
                          <td>
